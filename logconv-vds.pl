...
my $start;
if($startTime){
	if ($startTime =~ / *([0-9a-z:\/ ]+)/i){$start=$1;}
}
my $end;
if($endTime){
	if ($endTime =~ / *([0-9a-z:\/ ]+)/i){$end =$1;}
}
...
if ($logStart =~ / *( [0-9:]+)/i ){
        ...
        $timeHour = $timeComps[0] * 3600;
        $timeMinute = $timeComps[1] *60;
        $timeSecond = $timeComps[2];
...
if ($logEnd =~ / *( [0-9:]+)/i ){
        ...
        $endHour = $timeComps[0] * 3600;
        $endMinute = $timeComps[1] *60;
        $endSecond = $timeComps[2];
